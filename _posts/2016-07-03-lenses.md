...
### Создание и использование линзы
...
Получилось довольно немногословно, да? На самом деле, такую линзу можно было бы определить, используя встроенную в `ramda` функцию <a href="http://ramdajs.com/0.21.0/docs/#lensPath" target="_blank">lensPath</a>.
...
```
Есть некотoрый обьект, в котором есть юзеры с данными, а у юзеров есть члены семьи, у которых есть свои данные. У нас есть задача — написать функцию, которая будет обновлять имя юзера по `id`. Давайте решим эту задачу с помощью линз.
